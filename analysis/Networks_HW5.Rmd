...
## Try on your own.
Now that you have the basics, you can try by yourself with the data set of Kefi et al (2015,2016). It is a multiplex food web in which each layer represents a different kind of interaction: \
(1) trophic (chilean_TI.txt), \
(2) non-trophic positive (chilean_NTIpos.txt) and \
(3) non-trophic negative (chilean_NTIneg.txt). \
The nodes are common to all layersand their attributes are in: chilean_metadata.xls. The data set does not have interlayer edges so modularity can be run with a relax rate. See example in: https://ecological-complexity-lab.github.io/infomap_ecology_package/multilayer_relax.

Let's import data and preprocess it
```{r}
# Import data 
# 3 layers' links
# (1) trophic (chilean_TI.txt)
TI <- read.csv('data/chilean_TI.txt',sep='\t')
TI <- tibble::column_to_rownames(TI, var = "X")
TI <- TI %>% select(-X.1)
dim(TI)
head(TI)
colnames(TI) <- str_replace(colnames(TI), "X","")
TI <- as.matrix(TI)
# checking which index is not present
colnames(TI)
length(colnames(TI))
setdiff(c(1:110), colnames(TI)) # 21, 23, 24, 69 is not present
dim(TI)
head(TI)

# (2) non-trophic positive (chilean_NTIpos.txt)
NTI_p <- read.csv('data/chilean_NTIpos.txt',sep='\t')
NTI_p <- tibble::column_to_rownames(NTI_p, var = "X")
NTI_p <- NTI_p %>% select(-X.1)
dim(NTI_p)
head(NTI_p)
colnames(NTI_p) <- str_replace(colnames(NTI_p), "X","")
NTI_p <- as.matrix(NTI_p)
# checking which index is not present
colnames(NTI_p)
rownames(NTI_p)
length(colnames(NTI_p))
length(rownames(NTI_p))
setdiff(c(1:110), colnames(NTI_p)) # 21, 23, 24, 69 is not present
dim(NTI_p)
head(NTI_p)

# (3) non-trophic negative (chilean_NTIneg.txt)
NTI_n <- read.csv('data/chilean_NTIneg.txt',sep='\t')
NTI_n <- tibble::column_to_rownames(NTI_n, var = "X")
NTI_n <- NTI_n %>% select(-X.1)
dim(NTI_n)
head(NTI_n)
colnames(NTI_n) <- str_replace(colnames(NTI_n), "X","")
NTI_n <- as.matrix(NTI_n)
# checking which index is not present
colnames(NTI_n)
length(colnames(NTI_n))
setdiff(c(1:110), colnames(NTI_n)) # 21, 23, 24, 69 is not present
dim(NTI_n)
head(NTI_n)

# Nodes
nodes <- read_xls("data/chilean_metadata.xls")
colnames(nodes)
head(nodes)

nodes_formatted <- nodes %>% rename(node_name = `Species names`, node_group = trophic, node_id = Spec) %>% select(node_id, node_name, node_group)
head(nodes_formatted)

layer_map <- tibble(layer_id=1:3, layer_name=c("TI","NTI_p","NTI_n"))
```

Construct Intra layer edge list
```{r}
# Checking matrix type: this is a square matrix / binary matrix
dim(TI)
isSymmetric(as.matrix(TI)) # directed matrix

# Testing if it works: convert to edge_list
edge_TI <- create_monolayer_object(x=TI, directed = T, bipartite = F, node_metadata = nodes_formatted)
edge_TI$edge_list # conversion complete

# Checking if it contains nodes # 21, 23, 24, 69, which should not be in the edge list
edge_TI$edge_list %>% filter(from %in% c(21, 23, 24, 69))
edge_TI$edge_list %>% filter(to %in% c(21, 23, 24, 69))

# Make adjacency matrix to edge list for all 3 layers
mat_list <- list(TI, NTI_p, NTI_n)

edgedf_list <- list()
for (i in 1:length(mat_list)){
  edgedf_list[[i]] <- create_monolayer_object(x= mat_list[[i]], directed = T, bipartite = F, node_metadata = nodes_formatted, group_names=unique(nodes_formatted$node_group))
  # add column layer_from
  edgedf_list[[i]]$edge_list$layer_from <- i
  # add column layer_to
  edgedf_list[[i]]$edge_list$layer_to <- i
...
# Checking if it contains nodes # 21, 23, 24, 69, which should not be in the edge list
edgedf_list[[1]]$edge_list %>% filter(node_from %in% c(21, 23, 24, 69))
edgedf_list[[1]]$edge_list %>% filter(node_to %in% c(21, 23, 24, 69))
...
Construct Inter layer edge list.\
Oh..! The data set does not have inter-layer edges...

```{r}
# Let's see what nodes exist in each layer
# layer1_from <- df_intra %>% filter(layer_from ==1) %>% select(node_from) %>% unlist() %>% unique()
# layer1_to <- df_intra %>% filter(layer_from ==1) %>% select(node_from) %>% unlist() %>% unique()
# length(layer1_from)
# length(layer1_to)
# length(union(layer1_from,layer1_to))
# layer1_nodes <- union(layer1_from,layer1_to)
# 
# layer2_from <- df_intra %>% filter(layer_from ==2) %>% select(node_from) %>% unlist() %>% unique()
# layer2_to <- df_intra %>% filter(layer_from ==2) %>% select(node_from) %>% unlist() %>% unique()
# length(layer2_from)
# length(layer2_to)
# length(union(layer2_from,layer2_to))
# layer2_nodes <- union(layer2_from,layer2_to)
# 
# layer3_from <- df_intra %>% filter(layer_from ==3) %>% select(node_from) %>% unlist() %>% unique()
# layer3_to <- df_intra %>% filter(layer_from ==3) %>% select(node_from) %>% unlist() %>% unique()
# length(layer3_from)
# length(layer3_to)
# length(union(layer3_from,layer3_to))
# layer3_nodes <- union(layer3_from,layer3_to)
# 
# node_in_layer <- data.frame(node_id = nodes_formatted$node_id,
#            layer1 = ifelse(nodes_formatted$node_id %in% layer1_nodes,1,0), #Layer1 node exists:1; doesn't exist:0
#            layer2 = ifelse(nodes_formatted$node_id %in% layer2_nodes,1,0), #Layer1 node exists:1; doesn't exist:0
#            layer3 = ifelse(nodes_formatted$node_id %in% layer3_nodes,1,0) #Layer1 node exists:1; doesn't exist:0
#            )
# 
# head(node_in_layer)
```

Run modularity with infomap
```{r}
# Create a multilayer object
KEFI <- create_multilayer_object(extended = df_intra, nodes = nodes_formatted, 
                                 intra_output_extended = F,
                                 inter_output_extended = F)

#Run infomap
KEFI_modules <- run_infomap_multilayer(M=KEFI, relax = T,
                                       multilayer_relax_rate = 0.1,
                                       multilayer_relax_limit = NULL,
                                       multilayer_relax_limit_up = NULL,
                                       multilayer_relax_limit_down = NULL,
                                       flow_model = 'directed', 
                                       silent = T, trials = 100, seed = 497294, temporal_network = F)
...
```{r}
# (1) number of modules
KEFI_modules$m # 8 modules

# (2) Module size
df_size <- KEFI_modules$modules %>% group_by(module) %>% summarize(size = n_distinct(node_name)) %>% ungroup()
...
# (3) Module size per layer (number of physical nodes within each module within each layer).
df_mspl <- KEFI_modules$modules %>% select(module, layer_id, node_name) %>%  group_by(module,layer_id) %>% summarize(size = n_distinct(node_name)) %>% ungroup()

library(latticeExtra)
cloud(size~module+layer_id, df_mspl, panel.3d.cloud=panel.3dbars, col.facet='purple', 
      xbase=0.4, ybase=0.7, scales=list(arrows=FALSE, col=1),
      ylim = c(0.1,3.9),
      xlim = c(0.1,8.9),
...
```{r}
#Module persistance
modules_persistence <- KEFI_modules$modules %>%
  group_by(module) %>%
  summarise(b=min(layer_id), d=max(layer_id), persistence=d-b+1)

# Plot modules' persistence
plot_multilayer_modules(KEFI_modules, type = 'rectangle', color_modules = T)+
...
```{r}
#Plot species flow through modules in time
plot_multilayer_alluvial(KEFI_modules, module_labels = F)+
  scale_x_continuous(breaks=seq(0,3,1))+
